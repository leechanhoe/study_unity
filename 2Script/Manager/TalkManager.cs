...
public class TalkManager : MonoBehaviour
{
    ...
    {/*
        ...
        talkData.Add(3000, new string[] { "누군가 사용했던 흔적이 있는 책상이다." });

        //퀘스트 대화
        talkData.Add(10 + 1000, new string[] { "어서 와.:0", "이 마을에 놀라운 전설이 있다는데:1", "오른쪽 호수 쪽에 루도가 알려줄꺼야.:0" });
        talkData.Add(11 + 1000, new string[] { "아직 못 만났어? 루도는 호수 오른쪽에 있어.:1"});
    ...
    public string GetTalk(int id,int talkIndex)
    {
        if (!talkData.ContainsKey(id))//대화 없을 때 예외처리
        {
            if (!talkData.ContainsKey(id - id % 10))
                return GetTalk(id - id % 100, talkIndex); //퀘스트 맨 처음 대사가 없을 때 기본 대사 가져오기
            else
    ...
    public Sprite GetBackGround(int id, int BackgroundIndex)
    {
        return BackGroundArr[id + BackgroundIndex];
    }
}
